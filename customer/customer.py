...
def create_db():
  ...
  f = open("customer/customer_db.txt","a+")
  f.write(new_account + "\n")
...
def eval_login(account):
  # loopy process that loops through line by line -> customer_db.txt
  customer_db = open("customer/customer_db.txt", "r")
  lines = customer_db.readlines()
  count = 0
  #this_line
  for line in lines:
    count += 1
